...
server <- function(input, output, session) {
  ...
  output$weather_display <- renderText({
    ...
    # Print the weather data to the console for debugging
    print(weather)
    
    if (!is.null(weather)) {
      # Try to fetch individual weather attributes safely
      temp <- tryCatch({ weather$main$temp }, error = function(e) { NA })
      feels_like <- tryCatch({ weather$main$feels_like }, error = function(e) { NA })
      humidity <- tryCatch({ weather$main$humidity }, error = function(e) { NA })
      wind_speed <- tryCatch({ weather$wind$speed }, error = function(e) { NA })
      
      # Format the output to show each weather attribute
      weather_info <- paste(
        "Temperature: ", ifelse(!is.na(temp), paste(temp, "°C"), "Data not available"), 
        "\nFeels Like: ", ifelse(!is.na(feels_like), paste(feels_like, "°C"), "Data not available"),
        "\nHumidity: ", ifelse(!is.na(humidity), paste(humidity, "%"), "Data not available"),
        "\nWind Speed: ", ifelse(!is.na(wind_speed), paste(wind_speed, "m/s"), "Data not available")
      )
      return(weather_info)
    } else {
